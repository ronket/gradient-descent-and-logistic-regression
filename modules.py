...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        log_term = np.log(1 + np.exp(dot_X_w))
        return - (1 / X.shape[0]) * (np.sum(y*dot_X_w - log_term))
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        dot_X_w = X @ self.weights
        sig = np.exp(dot_X_w) / (1 + np.exp(dot_X_w))
        gd = X.T @ (y - sig) / X.shape[0]
        return -gd
class RegularizedModule(BaseModule):
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        return fidelity_jac + self.lam_ * reg_jac
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        if self.include_intercept_:
            self.regularization_module_.weights = np.r_[0, weights[1:]]
        else:
