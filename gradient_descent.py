...
class GradientDescent:
    ...
    def __init__(self,
                 ...
                 max_iter: int = 1000,
                 out_type: str = "last",
                 callback: Callable[[GradientDescent, ellipsis], None] = default_callback):
        """
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        """
        weights_sum =np.zeros_like(f.weights.shape)  
        old_weights = f.weights.copy()

        optimal_weights = f.weights
        best_val = np.inf 
        delta = self.tol_
        t = 0
        ...
        while t < self.max_iter_ and delta >= self.tol_:
            ...
            eta = self.learning_rate_.lr_step(f=f, x=X, y=y, gd=-grad, t=t)
            f.weights -= eta * grad

            # Update average weights
            weights_sum = weights_sum + f.weights

            # Update best weights
            if val < best_val:
                best_val = val
                optimal_weights = np.copy(f.weights)

            delta = np.linalg.norm(f.weights - old_weights)
            old_weights = np.copy(f.weights)            
            t += 1
